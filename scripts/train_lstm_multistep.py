"""
Train patient-specific LSTM models using multi-step MSE loss (Objective 2).
...
def main():
    ...
    lookback = 24  # 2 hour context; consistent with PatchTST baseline and Hüni [8]
    horizon = 12
    ...
    print(f"Training patient-specific LSTM (multi-step MSE) for {len(all_pids)} patients...")
    ...
    np.savez(
        "reports/results/lstm_multistep_traces_all_patients.npz",
        **{f"{pid}_true": traces[pid][0] for pid in pids_done},
        **{f"{pid}_pred": traces[pid][1] for pid in pids_done},
    )

    with open(
        "reports/results/lstm_multistep_per_patient_metrics_all.csv", "w", encoding="utf8"
    ) as f:
        ...
        for pid, r, la_r, m, h in zip(pids_done, rmses, lag_rmses, maes, hypo_metrics_list):
            f.write(
                f"{pid},lstm_multistep,{r:.6f},{la_r:.6f},{m:.6f},{h['precision']:.6f},{h['recall']:.6f},{h['fbeta']:.6f}\n"
            )
    ...
    with open("reports/results/lstm_multistep_summary.json", "w", encoding="utf8") as f:
        json.dump(summary, f, indent=2)

    print(f"\n=== LSTM multi-step MSE (test) ===")
