"""
Train patient-specific PatchTST binary classifiers for point-in-time
hypoglycemia detection at exactly 60 minutes ahead.

Mirror of train_lstm_point_event.py using the PatchTST backbone. See that
module for the full rationale and label design.

Label window: series[i+lookback+H-D : i+lookback+H+D+1]  (H=12, D=3)
y[i] = 1 if the minimum of that 7-step window is below 70 mg/dL.

Architecture and hyperparameters are identical to train_patchtst_event.py.
...
def make_binary_dataset(series, lookback, threshold):
    """
    ...
    ys = np.lib.stride_tricks.sliding_window_view(s, label_win)[
        lookback + H - D : lookback + H - D + n
    ]                                                   # shape (n, 7)
...
def train_patient(
    ...
    X_tr, y_tr   = make_binary_dataset(train_s, lookback, HYPO_THRESH)
    X_val, y_val = make_binary_dataset(val_s,   lookback, HYPO_THRESH)
...
@torch.no_grad()
def eval_patient(model, test_s, lookback, device):
    """Return (y_true_labels, y_pred_probs) for the test split."""
    X, y = make_binary_dataset(test_s, lookback, HYPO_THRESH)
    if X is None:
        return None, None
...
def main():
    ...
    print(f"Training PatchTST point-event classifier for {len(all_pids)} patients...")
    ...
    for i, pid in enumerate(all_pids):
        ...
        model = train_patient(pid, train_s, val_s, lookback, device, **best_hp)
        if model is None:
            continue

        y_true, y_prob = eval_patient(model, test_s, lookback, device)
        if y_true is None:
            continue
    ...
    with open("reports/results/patchtst_point_event_summary.json", "w", encoding="utf8") as f:
        json.dump(summary, f, indent=2)

    print(f"\n=== PatchTST Point-Event Classifier (test, H={H}, D=±{D}) ===")
