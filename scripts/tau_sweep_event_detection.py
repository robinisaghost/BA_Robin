"""
Objective 3: τ-Sweep for Hypoglycemia Event Detection

Evaluates all trained models across a range of time tolerance values
τ ∈ {1, 2, 3, 4, 5, 6} steps (= 5, 10, 15, 20, 25, 30 minutes).

For each (model, τ) combination, computes Precision, Recall, F1, and F2
...
that degrades sharply as τ decreases is only useful when ample warning time
...
def evaluate_model_tau(traces: dict, tau: int) -> tuple[float, float, float, float, list]:
    """
    Compute mean Precision, Recall, F1, F2 across all patients for a given τ.
...
def main():
    ...
    for model_name, npz_path in MODELS.items():
        ...
        for tau in TAU_VALUES:
            ...
            print(f"  tau={tau_min:2d}min  precision={prec:.4f}  recall={rec:.4f}"
                  f"  F1={f1:.4f}  F2={f2:.4f}")
