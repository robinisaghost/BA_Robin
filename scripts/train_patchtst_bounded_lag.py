...
def train_patient(pid, train_s, val_s, test_s, lookback, horizon, device,
    ...
    model = PatchTST(
        lookback=lookback, horizon=horizon, patch_len=patch_len, stride=stride,
        d_model=d_model, n_heads=n_heads, n_layers=n_layers, dim_ff=dim_ff, dropout=dropout,
    ).to(device)
...
def main():
    ...
    print(f"Training patient-specific PatchTST (bounded-lag loss, D={max_lag}) for {len(all_pids)} patients...")
    ...
    np.savez(
        "reports/results/patchtst_bounded_lag_traces_all_patients.npz",
        **{f"{pid}_true": traces[pid][0] for pid in pids_done},
        **{f"{pid}_pred": traces[pid][1] for pid in pids_done},
    )

    with open("reports/results/patchtst_bounded_lag_per_patient_metrics_all.csv", "w", encoding="utf8") as f:
        ...
        for pid, r, m, h in zip(pids_done, rmses, maes, hypo_metrics_list):
            f.write(f"{pid},patchtst_bounded_lag,{r:.6f},{m:.6f},{h['precision']:.6f},{h['recall']:.6f},{h['fbeta']:.6f}\n")
    ...
    with open("reports/results/patchtst_bounded_lag_summary.json", "w", encoding="utf8") as f:
        json.dump(summary, f, indent=2)

    print("\n=== PatchTST bounded-lag (test) ===")
