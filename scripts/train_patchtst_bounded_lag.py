...
Identical pipeline to train_patchtst_60min.py with two differences:
...
def train_patient(
    ...
    train_ds = MultiPatientWindowDataset(
        {pid: train_s}, [pid], lookback=lookback, horizon=horizon_ext
    )
    val_ds = MultiPatientWindowDataset(
        {pid: val_s}, [pid], lookback=lookback, horizon=horizon_ext
    )
...
def main():
    ...
    with open(
        "reports/results/patchtst_bounded_lag_per_patient_metrics_all.csv",
        "w",
        encoding="utf8",
    ) as f:
        ...
        for pid, r, la_r, m, h in zip(pids_done, rmses, lag_rmses, maes, hypo_metrics_list):
            f.write(
                f"{pid},patchtst_bounded_lag,{r:.6f},{la_r:.6f},{m:.6f},{h['precision']:.6f},{h['recall']:.6f},{h['fbeta']:.6f}\n"
            )
