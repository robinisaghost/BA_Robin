...
from ba_baseline.metrics.metrics import best_lag_crosscorr
...
def main():
    ...
    # --- load summary ---
    with open("reports/results/lstm_1step_summary.json", "r", encoding="utf8") as f:
        summary = json.load(f)

    # --- load lags ---
    lags = []
    pids = []
    with open("reports/results/lstm_1step_lags.csv", "r", encoding="utf8") as f:
        next(f)
        for line in f:
    ...
    plt.tight_layout()
    plt.savefig("reports/figures/lstm_lag_hist_1step.png", dpi=200)
    plt.close()

    # 2) Example overlay plots
    data = np.load("reports/results/lstm_1step_traces_examples.npz")
    ...
    for pid in example_pids:
        ...
        lag = best_lag_crosscorr(t, p, max_lag=24)
        p_shift = shift_series(p, lag)
        print(
            pid,
            "true std",
            float(np.std(t)),
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(f"reports/figures/lstm_overlay_patient_{pid}_1step.png", dpi=200)
        plt.close()

    # 3) Save a small text summary for easy copy into report
    with open("reports/figures/lstm_1step_summary.txt", "w", encoding="utf8") as f:
