"""
Train patient-specific PatchTST models using multi-step MSE loss (Objective 2).

Identical pipeline to train_patchtst_60min.py with one difference:
  The model predicts all horizon=12 future steps and the MSE loss is computed
  over all 12 steps jointly, not only the 60-min endpoint.

The hypothesis is that intermediate supervision over the full trajectory keeps
the model "in phase" over longer horizons, reducing the time-shift artefact
observed under single-point training [7].
...
ensures that any difference in results is attributable solely to the training
objective, consistent with the ablation design of van den Hoek [7].
...
def train_patient(
    ...
    # horizon=12: model predicts the full trajectory for multi-step supervision.
    model = PatchTST(
        lookback=lookback,
        horizon=horizon,
        patch_len=patch_len,
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for x, y in train_loader:
            ...
            opt.zero_grad()
            # Multi-step loss: MSE over all 12 horizon steps jointly.
...
def main():
    ...
    # in results is attributable to the training objective only [7].
    best_hp = {
        "d_model": 64,
        "n_heads": 4,
        "n_layers": 3,
    ...
    print(f"Training patient-specific PatchTST (multi-step MSE) for {len(all_pids)} patients...")
    ...
    np.savez(
        "reports/results/patchtst_multistep_traces_all_patients.npz",
        **{f"{pid}_true": traces[pid][0] for pid in pids_done},
        **{f"{pid}_pred": traces[pid][1] for pid in pids_done},
    )

    with open(
        "reports/results/patchtst_multistep_per_patient_metrics_all.csv",
        "w",
        encoding="utf8",
    ) as f:
        ...
        for pid, r, la_r, m, h in zip(pids_done, rmses, lag_rmses, maes, hypo_metrics_list):
            f.write(
                f"{pid},patchtst_multistep,{r:.6f},{la_r:.6f},{m:.6f},{h['precision']:.6f},{h['recall']:.6f},{h['fbeta']:.6f}\n"
            )
    ...
    summary = {
        ...
        "target_minutes": 60,
        "model": "patchtst_multistep",
        "lookback": lookback,
        "hyperparameters_source": "baseline_optuna_patient_85202",
        "hyperparameters": best_hp,
    }

    with open("reports/results/patchtst_multistep_summary.json", "w", encoding="utf8") as f:
        json.dump(summary, f, indent=2)

    print("\n=== PatchTST multi-step MSE (test) ===")
