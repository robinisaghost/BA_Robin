...
def main():
    ...
    print(f"Training patient-specific PatchTST (Soft-DTW) for {len(all_pids)} patients...")
    ...
    with open(
        ...
        for pid, r, m, h in zip(pids_done, rmses, maes, hypo_metrics_list):
            f.write(
                f"{pid},patchtst_dtw,{r:.6f},{m:.6f},{h['precision']:.6f},{h['recall']:.6f},{h['fbeta']:.6f}\n"
            )
    ...
    summary = {
        ...
        "target_minutes": 60,
        "model": "patchtst_dtw_per_patient",
        "lookback": lookback,
        "patch_len": 12,
        "stride": 6,
        "d_model": 64,
    ...
    print("\n=== PatchTST Soft-DTW per-patient (test) ===")
