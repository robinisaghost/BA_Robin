...
sole difference that the training and validation loss is replaced by soft_dtw
...
def main():
    ...
    print(f"Training patient-specific PatchTST (Soft-DTW, gamma={GAMMA}) for {len(all_pids)} patients...")
    ...
    with open("reports/results/patchtst_dtw_per_patient_metrics_all.csv", "w", encoding="utf8") as f:
        ...
        for pid, r, m, h in zip(pids_done, rmses, maes, hypo_metrics_list):
            f.write(f"{pid},patchtst_dtw,{r:.6f},{m:.6f},{h['precision']:.6f},{h['recall']:.6f},{h['fbeta']:.6f}\n")
