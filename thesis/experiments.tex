...
Across both strategies, Tab.~\ref{tab:exp:obj1} reveals a consistent pattern: in every architecture--loss combination, RMSE$^*$ rises above the MSE baseline even as the mean lag falls, so the partial temporal realignment is bought at a cost to lag-adjusted trajectory accuracy that grows with the aggressiveness of the tolerance, and neither strategy closes the detection gap observed at baseline.
