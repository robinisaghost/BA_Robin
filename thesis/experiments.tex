...
\centering
\small
\caption{Comprehensive summary of all eight training configurations. Bold values mark the best result per column. Lag is the average absolute shift in minutes that minimises per-patient RMSE ($|\Delta^*| \times 5$~min). RMSE and MAE are in mg/dL; Hypo-F1 is derived from threshold-based event detection at 70~mg/dL.}
\label{tab:exp:overall}
\begin{tabular}{llrrrr}
\toprule
Architecture & Training & RMSE\,$\downarrow$ & Lag\,$\downarrow$ (min) & MAE\,$\downarrow$ & Hypo-F1\,$\uparrow$ \\
\midrule
