...
The two classifiers therefore exhibit different alarm-timing profiles. The LSTM distribution, dominated by the zero-lead-time bin, is consistent with a classifier that responds primarily when glucose values are already at or near the threshold at alarm time; the PatchTST distribution, flatter and shifted toward positive lead times, suggests alarms triggered more often by glucose patterns that precede the crossing. These profiles add a dimension to the classifier comparison that the aggregate F2 values in Tab.~\ref{tab:exp:classifiers} do not convey on their own.

Because a preventive response may itself require time, as the pharmacodynamic delays of Chapter~\ref{ch:introduction} suggest, alarm timing relative to event onset is a relevant property alongside recall; what the distributions imply for each classifier's practical utility is discussed in Chapter~\ref{ch:conclusions}.
...
Across the ten configurations evaluated in this chapter, none leads on both trajectory quality and event detection, and four patterns recur. First, RMSE, lag, and RMSE$^*$ move independently: every Objective~1 configuration that reduces the mean lag also increases RMSE$^*$, while the most accurate configuration (LSTM multi-step) leaves it essentially unchanged. Second, the configuration that forecasts best does not detect best: the F2 scores of all eight forecast-derived configurations remain well below the direct classifiers. Third, the two architectures diverge consistently, from the asymmetric response to Soft-DTW to the performance inversion between forecasting and classification. Fourth, the direct classifiers close the detection gap on recall and F2, but the lead-time analysis shows that detection and anticipation are distinct properties that F2 alone does not separate.
