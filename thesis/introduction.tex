...
Deep learning models trained with mean squared error (MSE) on the glucose signal systematically produce predictions that are structurally correct but displaced forward in time by approximately 50 to 55~minutes, an effect first observed in preliminary PRG experiments~\citep{prg_glucose_proposal}. The cause is the interaction between the pointwise loss and the strongly autocorrelated CGM signal, which rewards reproducing recent history rather than anticipating change~\citep{wolff2025cgpm}; the mechanism is derived formally in Section~\ref{sec:bg:losses:mse}. The result is a prediction curve that mirrors the true trajectory at a temporal offset, as illustrated in Fig.~\ref{fig:intro:timeshift_demo}.
...
Closely related work at the University of Bern is the bachelor thesis of \citet{huni2023glucose}, which evaluated LSTM and Graph Attention Networks~\citep{velickovic2018gat} for hypoglycaemia prediction on CGM data but did not isolate or quantify the time-shift artefact that is the primary focus of the present work. At the architecture level, \citet{lim2021tsforecast} survey the progression from recurrent models to attention-based architectures~\citep{vaswani2017attention}, of which PatchTST~\citep{nie2023patchtst} is the patch-based Transformer adopted here (detailed in Section~\ref{sec:bg:models:patchtst}).

In the blood glucose domain, \citet{fox2018multioutput} demonstrate that supervising on the full future trajectory rather than a single endpoint improves forecast accuracy, forming the basis of Objective~2. For direct event detection, \citet{shao2024hypoglycemia} show that a deep-learning classifier predicts hypoglycaemia more accurately than traditional machine-learning baselines, motivating the dedicated classification approach of Objective~3; \citet{cinar2025hypoclassification} further demonstrate deep-learning hypoglycaemia classification across multiple prediction horizons. Regarding the time-shift specifically, \citet{wolff2025cgpm} show that accuracy metrics such as RMSE reward trivial models that merely reproduce the most recent observation; their contribution is diagnostic, establishing the delay as a real and under-recognised problem but not evaluating training-level strategies to reduce it. \citet{sun2026futureaware} address the delay through future-aware training that injects privileged future-disturbance information, a different intervention from the offset-aware and event-centric strategies examined here. The present thesis takes the problem identified by these works as its starting point and contributes what they do not: a controlled ablation of concrete mitigation strategies, evaluated with a dedicated lag-adjusted metric (RMSE$^*$) that isolates the timing component of the error from genuine shape inaccuracy.

The experiments are conducted on the T1DATA dataset of \citet{garciatirado2023t1d}, comprising CGM records from 36 individual adults with Type~1 diabetes recorded over approximately 30 days per patient at a 5-minute sampling interval. All models are trained and evaluated independently per patient; cohort means are reported throughout. Two architectures serve as parallel baselines: a recurrent Long Short-Term Memory network and the patch-based PatchTST Transformer. A strictly univariate input is used throughout to deliberately isolate the temporal predictive capacity of the glucose signal itself. Exactly one configuration variable is modified per research objective, enabling clean attribution of any observed differences in performance.
